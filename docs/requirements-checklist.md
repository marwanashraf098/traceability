...
- [x] 7.1 [M] Pipeline list + filters (incl. Self-Pickup Pending); search by number/name/phone/tracking [2026-08-16: added `GET /orders/summary` — 5-tile all-time calm row (Total/Processing/With courier/Delivered/Returned) above the filter bar, bucketed server-side via the same `OrderStatusDeriver.derive()` `funnel()` already uses (no client-side scraping, no second derivation path); see PROGRESS.md "Orders — status-split diagnosis + all-time summary tile row"]
- [x] 7.2 [M] Order detail: customer, lines, COD, status timeline, allocated pieces, shipment, audit [2026-08-05: FR-7/FR-11 A1–A2 — single derived `<OrderStatus>` headline (`OrderStatusDeriver`) replaces the old independent pipeline STATUS pill + shipment SHIPMENT pill; shipment card is facts-only now. 2026-08-05 (same day): A3 cancelled-conflict flag (`conflictKey`, health chips suppressed on cancelled orders) + A4 collapsed forward-leg timeline (consecutive-identical-state folding, exception/terminal rows never folded) — see docs/order-status-redesign-build-spec.md. 2026-08-05 (later same day): A3.1 reversed and built — return-leg ShipmentCard now shows a leg-scoped badge (`OrderStatusDeriver.deriveLegStatus()`, no order-level precedence), since CRP return-leg shipments are real production code, not a hypothetical. Still open: Part B (no exception for the conflict chip to link to yet); a dedicated return-leg timeline (A4 kept it on raw/ungrouped history). 2026-08-16: Orders list + detail restyled to `design/Traced Orders Flow v2.dc.html` (Design System v1.0, frontend-only) — added a read-only 5-step status stepper (Created→Packed→With courier→In transit→Delivered) on the detail page, suppressed for cancelled/not-traced/off-ladder statuses and the one delivery_failed+exception edge case with no reliable rank signal; see PROGRESS.md "Orders screen restyle" for the full mapping, the 5 cuts (row-expansion, 6-tile metric row, Amount payment sub-line, "Synced Xm ago", "Out for delivery" step — none had an honest frontend-only data source), and the tripwire test methodology. 2026-08-16: a follow-up status-split request (should `label_created` outrank `packed`, or vice versa?) was diagnosed but NOT built — `label_created` and `packed` turned out to be two independently-arriving signals, not a fixed sequence (Mode-B webhook auto-match can set `label_created` before Traced ever packs the order); needs an explicit design decision before `OrderStatusDeriver`/the stepper mapping is touched again — see PROGRESS.md "Orders — status-split diagnosis" for the full finding. 2026-08-16, later same day: **Slice 1 built** — a shipment record existing is not proof packing happened; added purely-additive `OrderStatusDeriver.DerivedOrderStatus.packedConfirmed` (no `PROGRESS_RANK` reorder, no migration), fixing the phantom "Packed ✓" checkmark on the detail stepper and the merchant-facing "With courier"/Courier miscount in `funnel()`/`summary()`; see PROGRESS.md "Orders — Slice 1: phantom-packed fix" for the design, the monotonicity proof, and the deploy-boundary note]
...
- [x] 15.3 [M] Exceptions center, one prioritized list w/ resolving actions: lost · never-received · unexpected return · failed attempts · stuck (5-day default, per-tenant) · Bosta state 103 limbo · NDR 26–30 evidence · short (signal missing — FR-7.7 deferred) · blocked customer · address review · pending unpack · self-pickup no-show · missing AWB · unlinked Mode-B · shopify_cancel_vs_inflight (Day 25) · Shopify edit conflict · cancelled_live_shipment + cancelled_but_delivered (2026-08-05, Part B of the order-status redesign — self-resolving, scoped to latest forward shipment; see docs/order-status-redesign-build-spec.md); archived w/ resolver
...
Built against `docs/transfers-build-spec.md` (renumbered from a provisional FR-21 — FR-21 is Stock Taking, already built). `closeTransfer` landed in 22.4 rather than 22.5 per explicit build-order request — 22.5 became `reprintOutstandingLabels` only. 22.6 completes `TransferController`'s HTTP surface: the "command" family (create/begin-reconcile/classify/close) throws `TransferException` mapped to `{code, message_en, message_ar}` by the single handler added in FR-22.3 (not a second one); the "scan" family (scan-out/scan-back) instead returns `ScanOutResult`/`ScanBackResult` directly as 200 JSON, mirroring `FulfillController`'s `/scan` (Invariant 7 — same shape, frontend translates by `code`). `listOpen`/`getTransfer` registered in `RlsCoverageTest` COVERED with seeded tests. A follow-up commit closed a real cross-tenant leak in `createTransfer()` (FK checks bypass RLS) and made the scan family bilingual too (was English-only, unlike `TransferException`). 22.7 found (not fixed — flagged in PROGRESS.md) that `CatalogController`'s `ALL_STATUSES`/`pieceCounts` breakdown predates FR-22 and silently drops `out_on_transfer`/`sold` pieces from its "total" tally — the `on_hand`/`available` formula itself is unaffected (separate, explicit predicate), only the raw per-status breakdown undercounts; fixed in its own follow-up commit. 22.8: no new guard code — mirrors the self-pickup precedent exactly (confirmed by grep: neither BostaWebhookJob nor ShipmentLinkService has any explicit self-pickup or transfer check). BostaWebhookJob relies on `InventoryLedger.ALLOWED` (no `out_on_transfer:*` entry for any Bosta target → `IllegalTransitionException`, caught, logged, webhook still marked 'processed'); `ShipmentLinkService.transitionPackedPieces()` pre-filters `p.status = 'packed'` in SQL, an even earlier guard. Correction to the spec wording: neither path currently writes to a distinguishable "exceptions list" for this case — it's `log.warn` only, exactly matching self-pickup's own existing treatment; not a silent drop or a stacktrace, but not a literal exceptions-center row either.

## FR-23 Overview Dashboard
- [x] 23.1 [M] Live per-status piece totals (`GET /inventory/status-totals`, pure per-status GROUP BY, distinct from the 30-day-windowed Group B) + low-stock/inventory-value (`GET /inventory/valuation`, kept off the hot per-status endpoint on purpose)
- [x] 23.2 [M] `/returns/pending` → `{items, total}` (was a bare paginated list with no total — awaiting-inspection tile reads `.total`, never page length)
- [x] 23.3 [M] Fulfillment funnel — today (`GET /orders/funnel`), server-side via `OrderStatusDeriver.derive()` on the same raw rows `list()` already fetches; no client-side re-derivation [2026-08-16: Courier-bucket miscount fixed — a `label_created`/`awaiting_courier` order with `packedConfirmed==false` (shipment exists, packing never happened) now routes to New/Picking by its own raw status instead of Courier; see PROGRESS.md "Orders — Slice 1: phantom-packed fix"]
- [x] 23.4 [M] Throughput — 30d received vs handed-to-courier (`GET /inventory/throughput`), from `piece_events` (`event_type='received'` / `to_status='with_courier'`), not `/orders/daily-counts`
- [x] 23.5 [M] Recent activity feed (`GET /activity/recent`) — finalized receiving (`receipts.kind='inbound'`) + finalized stock-take sessions only; pickups excluded (no actor column, see PROGRESS.md)
- [x] 23.6 [M] `/exceptions/count` severity split — backward-compatible `{count, critical, warning}` (4-tier severity collapsed to 2 buckets), shell nav badge unaffected
- [x] 23.7 [M] Onboarding: condensed 4-of-5-step dismissible card on Overview (`tenants.onboarding_dismissed_at`, `POST /onboarding/dismiss`) — hides on dismiss OR allDone, independent conditions; full 5-step checklist page unchanged
- [x] 23.8 [M] Fresh-tenant card — replaces the whole zone stack when total pieces = 0 and onboarding not all done
- [x] 23.9 [M] Frontend: full Overview rebuild — `MeContext`/`useMe()` (Layout sole provider), `MiniStat`, `SectionHeader` optional-note, per-zone independent loading/calm-empty/minimal-error, donut/funnel/severity/throughput/activity widgets, inventory-value "not set up" empty state (M=0 variants costed); old Group A/B tiles + orders line-chart removed from this screen (`OrdersChart` kept as a reusable component, unused elsewhere); AR+EN, RTL logical props
- [x] 23.10 [M] Migrations V70–V72: `variants.unit_cost`, `tenants.low_stock_threshold` (default 5), `tenants.onboarding_dismissed_at` — additive columns on already-RLS'd tables, no new policy needed

Built 2026-08-13 on branch `overview-rebuild`, single cutover, not yet merged to main/pilots. Mockup: `design/Traced Overview Dashboard.dc.html`. `RlsCoverageTest` COVERED with seeded tests for all 5 new GET endpoints (`inventory/status-totals`, `inventory/valuation`, `orders/funnel`, `inventory/throughput`, `activity/recent`); the pre-existing `returnsPending_returnsSeededPiece` test updated for the new `{items,total}` shape. Full backend suite green (995 tests). Frontend: `inventory.test.tsx` (5 tests, all against the now-removed Group A/B tiles) deleted as dead-code cleanup; `overview.test.tsx` rewritten, 12 tests, Layout-wrapped via `stubFetchWithShellDefaults` (now with an `overrides` param for `/me` and `/exceptions/count`). Verified live end-to-end (real Postgres + Spring Boot + browser) in EN/LTR and AR/RTL with both a long Latin name and a long Arabic name.

## FR-24 Returns — Session-Based Rebuild
...
- [x] 24.7 [M] Reprint: new `POST /sessions/{id}/pieces/{pieceId}/reprint-label` — any piece status, reads stored barcode, never mints; old single-piece gated endpoint (`GET /pieces/{pieceId}/label`, return_pending_inspection/damaged only) left untouched as a separate surface, per the transfers-spec precedent of never widening it
- [x] 24.8 [M] Analytics: `GET /returns/analytics` — totals/restocked/damaged/mismatch/expected-not-scanned (wraps `neverReceived()` unchanged) + unassigned-pending count/list, derive-on-read
- [x] 24.9 [M] Receiving kind-filter retrofit (change A): `ReceivingService.requireOpen()`/`getSession()` now filter `kind='inbound'` (previously only `deleteSession()` did) — closes the shared-`receipts`-table gap from the other side; regression test proves a `kind='returns'` row is invisible to both
- [x] 24.10 [M] `ReturnController`/`ReturnService` trimmed: `GET /returns/pending` kept unchanged (Overview's Awaiting Inspection tile depends on it — semantically distinct from analytics' unassigned-pending count, not repointed); `listPending()`, `intakeScan()`, `GET /returns/intake`, `GET /returns/never-received`, per-piece `POST /returns/pieces/{id}/restock|damage` retired with the three-tab UI (no surviving callers, confirmed by grep)
- [x] 24.11 [M] New exception type `return_mismatch` added to `ExceptionService` (18th detector) — hardcoded bilingual EN/AR strings, matching the existing 17 types' precedent (the `exceptions.*` i18n gap is a tracked follow-up, not touched here)
- [x] 24.12 [M] Frontend: full `Returns.tsx` rebuild — route unwrapped at `/returns` (Fulfill's structure, not Receiving's — `<Layout>` applied internally around the landing view only), landing (analytics band, unassigned-pending callout, sessions table + pagination, already-open resume state) + full-screen immersive open-session scan loop (needs-decision/unexpected/illegal-mismatch-only/dispositioned/awaiting-scan/rejected-scan/close-blocked/close-summary), abandon confirm modal, AR+EN, RTL logical props

Built 2026-08-15 on branch `returns-rebuild`, single cutover, not yet merged. Mockup: `design/Traced Returns Flow v2.dc.html` (behavioral contract overrides the mockup wherever they differ — see `docs/fr-24-returns-session-rebuild-build-spec.md`). `MigrationSmokeTest` bumped 71→72 migrations / 29→32 `TENANT_SCOPED_TABLES`; `NotTracedBackfillTest` bumped 16→17. `RlsCoverageTest` COVERED updated (`/returns/sessions/{sessionId}`, `/returns/analytics` added; `/returns/never-received`, `/returns/sessions/{sessionId}/pieces` removed with the endpoints they tested; `returnsSessions`/new `returnsSessionDetail`/`returnsAnalytics` tests rewritten against the new tables). New dedicated RLS proof `ReturnSessionsRlsTest` (TransferRlsTest-style negative+positive-control pairs, all 3 tables). `Day12Test`/`ReturnSessionTest` adapted (old waybill-first cases rewritten or retired; `detectReturnInTransitStuck` detector cases kept unmodified). Full backend suite green (1014 tests, 0 failures/errors, 3 pre-existing skips). Frontend: `returns.test.tsx` fully rewritten (24 tests, was 11 against the retired three-tab UI); full suite at the established 3-pre-existing-failure baseline (`blocklist.test.tsx` fb7 + `overview.test.tsx` ov2/ov5 — matches every prior PROGRESS.md entry; a same-day `git stash` check had anomalously read 1, corrected the next day by re-running 3× against both the committed state and this branch's own head, identical 3-failure result both times, so the 1-reading was a flake, not a real baseline shift). Verified live end-to-end (real local Postgres via `docker-compose.yml`, not the Supabase-pointing `.env` — kept fully local/reversible per "no deploy"; Spring Boot + Vite dev + browser) in EN/LTR and AR/RTL with a long Latin name and a long Arabic name: landing populated/already-open/unassigned-callout, full scan→disposition→close cycle (legal/unexpected/both illegal-state sub-cases/AWB hub-prefix normalization/reprint-any-status), abandon→resurfaces-as-unassigned confirmed against the DB directly, close summary receipt. One live finding diagnosed as seed-data (bare-digit tracking-number storage vs. a test literal carrying a fake "QA-" prefix) confirming `TrackingNumberNormalizer` behaves correctly, not an app bug.
