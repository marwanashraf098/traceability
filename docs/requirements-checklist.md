...
- [x] 23.3 [M] Fulfillment funnel — today (`GET /orders/funnel`), server-side via `OrderStatusDeriver.derive()` on the same raw rows `list()` already fetches; no client-side re-derivation
