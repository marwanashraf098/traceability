...
All application writes run under `app_user` + row-level security. The eight functions below are the ONLY points where RLS is bypassed (hatches #1–#7) or, for hatch #8, where a `REVOKE`-restricted table is read on the caller's behalf. Each has an explicit justification. Any new cross-tenant read or write requires a new named function with the same scrutiny; no bare `BYPASSRLS` connections in application code.
