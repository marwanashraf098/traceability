...
All application writes run under `app_user` + row-level security. The fourteen entries below are the ONLY points where RLS is bypassed (hatches #1–#7, #9, #13, #14) or, for hatches #8, #10, #11, and #12, where a `REVOKE`-restricted table is read/swept, or an explicit BYPASSRLS admin connection is used, on the caller's behalf. Each has an explicit justification. Any new cross-tenant read or write requires a new named function (or, for #11's pattern, an equally-scrutinized admin-connection job) with the same scrutiny; no other bare `BYPASSRLS` connections in application code.
...
| 7 | `verify_and_consume_reset_code(p_email text, p_code_hash text)` | V85, extended V86 | Code-based forgot/reset-password: the caller has only an email + a 6-digit code, no tenant GUC yet. Resolves the active user by email (same basis as `auth_lookup_user`), then `SELECT ... FOR UPDATE` on that user's newest unconsumed/unexpired `password_reset_codes` row, atomically checking the 5-attempt lockout, comparing the code hash, and consuming on match. Returns `(user_id, tenant_id)` on success; empty result for every invalid sub-condition (no such active user / no eligible code / locked / hash mismatch) — no oracle, mirrors `consume_magic_link`. **V86** folded the invalidate-every-other-outstanding-code cleanup into this same function (atomic with the match) and, alongside hatch #8, made `password_reset_codes` grant-restricted like `magic_link_tokens` (see hatch #6) — `app_user` now has INSERT only; empirically confirmed before the fix that a tenant-A `app_user` session could `SELECT` tenant-B's rows directly (no RLS exists on this table by design — pre-session surface). |
| 8 | `check_password_reset_throttle(p_user_id uuid)` | V86 | Read-only companion to hatch #7: the request-throttle check (`PasswordResetService.isThrottled`) needs to read `password_reset_codes`, which `app_user` no longer has direct `SELECT` on after V86's `REVOKE`. Returns the count of codes issued to `p_user_id` in the last hour and the most recent issuance time; the 60-second debounce comparison stays in Java. `p_user_id` is always a value `PasswordResetService` already resolved via `auth_lookup_user` — never raw request input — so the absence of tenant scoping here is not an attacker-controllable probe. `LANGUAGE sql`, read-only, no atomicity concerns. |
| 9 | `consume_shopify_oauth_state(p_nonce text, p_callback_shop text)` | V87 | OAuth callback consumption is pre-session — the caller has only the state nonce, no tenant GUC yet, and Path-2 rows carry `tenant_id IS NULL` until the resolve-or-create decision runs. `SELECT ... FOR UPDATE` on the unguessable 128-bit nonce PK, then one empty result for every invalid sub-condition (not-found / already-consumed / expired past the hardcoded 600s TTL / shop-mismatch) — no oracle, mirrors `consume_magic_link`/hatch #7. Returns `(tenant_id, shop_domain)`; `tenant_id` is genuinely nullable and passes through unchanged for Path-2 — `ShopifyOAuthService.linkOrProvision()`'s entire resolve-or-create branch depends on receiving a real Java `null` here. Alongside hatch #10, made `shopify_oauth_state` grant-restricted like the other two pre-session tables — `app_user` now has INSERT only; empirically confirmed before the fix that a tenant-A `app_user` session could `SELECT` tenant-B's `shop_domain`, and the Path-2 null-tenant rows, directly. |
| 10 | `purge_expired_shopify_oauth_state()` | V87 | The hourly cleanup sweep (`ShopifyStateCleanupJob`) is deliberately unscoped — one `DELETE` for every tenant's (and every Path-2 null-tenant's) expired rows, no per-tenant GUC exists for this job to filter by. No auth logic, no branching — a pure janitorial `DELETE`, wrapped in `DEFINER` (rather than granting `app_user` a standing `DELETE`) so `app_user`'s privilege on `shopify_oauth_state` stays at zero beyond `INSERT`, matching the other two locked-down tables. Returns the deleted row count (`GET DIAGNOSTICS`) so the job's existing log line is unchanged. |
| 11 | `DemoSeeder.reseed()` (Java, not a SQL function) | V93, approved (FR-DEMO Day 1) | The demo-tenant re-seed is the single most dangerous operation in the demo feature: it deletes every mutable row for one tenant and reloads a fixture, in one transaction. Its FIRST statement — `SELECT id FROM tenants WHERE is_demo = true` — is a genuine cross-tenant read (it must see every tenant's `is_demo` flag to hard-assert exactly one demo tenant exists, before trusting any tenant id), which is impossible under `app_user` + RLS scoped to a single GUC value. Rather than a new SQL `SECURITY DEFINER` function, this hatch is a Java admin job running on the same `ownerDataSource` (`@FlywayDataSource`, BYPASSRLS, 2-connection Flyway/JobRunr pool) that `ExceptionDigestJob` and `ExceptionImmediateAlertJob` already use for their own `SELECT id FROM tenants` — no new connection pool, no new elevated credential. After the assert, every subsequent statement in the same transaction is still explicitly `WHERE tenant_id = :demo` (41 `DELETE`s in FK-safe order, then the fixture `INSERT`s) — the BYPASSRLS connection is what makes the initial assert possible, not a license to skip the per-statement filter. Triple hard-assert before any write: exactly one `is_demo=true` row, not one of the two known production pilot tenant ids, and equal to the fixed `DemoSeeder.DEMO_TENANT_ID` constant — any failure aborts with nothing deleted. |
| 12 | `check_demo_rate_limit(p_ip inet)` | V94, approved (FR-DEMO Day 2) | `POST /api/v1/public/demo/start` is unauthenticated and pre-session — the caller has no tenant context, and `demo_leads` (V93) carries no `tenant_id` to scope a read by even if one existed. `app_user` has INSERT-only on `demo_leads`; this function is the read-only companion that lets the rate-limit decision see counts without granting `app_user` direct `SELECT` — same shape as `check_password_reset_throttle` (hatch #8). Returns two independent counts (per-IP in the last hour, global in the last 90 minutes); the threshold comparison itself (5/hour per IP, 20/90-min global) stays in Java in `DemoStartService`, same split as hatch #8's 60-second debounce. `LANGUAGE sql`, read-only, no atomicity concerns. |
| 13 | `upgrade_custom_app_to_oauth(p_shop_domain, p_owner_email, p_shop_name, p_access_token_encrypted, p_refresh_token_encrypted, …)` | V31, amended V32 | Omitted from this table until 2026-09-23 (inventory correction — the function has been `SECURITY DEFINER` since V31, whose own header calls it an approved hatch). Option-B of the custom_app → public-OAuth migration: when a `shop_domain` already connected as `custom_app`/`custom_app_cc` installs via public OAuth, atomically creates a new tenant + owner user, upgrades the existing `stores` row in place (same `store_id`, so child FKs are preserved), and re-assigns that store's tenant-scoped child rows to the new tenant. Impossible under `app_user`+RLS for the same chicken-and-egg reason as #5 (the new tenant's UUID does not exist before its INSERT) plus a cross-tenant re-assignment. As of 2026-09-23 no application code calls it (Option-A, disconnect + reinstall, is the operational procedure); it exists so Option-B is not blocked later. |
| 14 | `resolve_tenant_by_portal_slug(p_slug text)` | V100 | **BUILT 2026-09-23 (Step 4a; approved 2026-09-23).** `SELECT id FROM tenants WHERE portal_slug = lower(p_slug) AND portal_enabled` — `LANGUAGE sql`, `SET search_path = public`. The customer-facing returns portal is unauthenticated and pre-session: the caller has only a public tenant slug, no GUC. Returns `tenant_id` only (one nullable `uuid`, read-only) — nothing else crosses the boundary. All subsequent portal work runs via `TenantContext.runAs(tenantId, …)` under `app_user` + RLS, exactly like the Bosta webhook's use of #4. |

**`EXECUTE` granted to `app_user` only; `REVOKE ALL … FROM PUBLIC` precedes every grant. Hatch #11 is the one exception to "SQL function" — see its row for why.**
