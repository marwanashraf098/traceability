...
All application writes run under `app_user` + row-level security. The seven functions below are the ONLY points where RLS is bypassed. Each has an explicit justification. Any new cross-tenant read or write requires a new named function with the same scrutiny; no bare `BYPASSRLS` connections in application code.
...
| 7 | `verify_and_consume_reset_code(p_email text, p_code_hash text)` | V85 | Code-based forgot/reset-password: the caller has only an email + a 6-digit code, no tenant GUC yet. Resolves the active user by email (same basis as `auth_lookup_user`), then `SELECT ... FOR UPDATE` on that user's newest unconsumed/unexpired `password_reset_codes` row, atomically checking the 5-attempt lockout, comparing the code hash, and consuming on match. Returns `(user_id, tenant_id)` on success; empty result for every invalid sub-condition (no such active user / no eligible code / locked / hash mismatch) — no oracle, mirrors `consume_magic_link`. Unlike hatch #6, `password_reset_codes` itself is NOT grant-restricted — issuance and the request-throttle count use plain `app_user` reads/writes (V1's default privileges, unrevoked, same actual posture `magic_link_tokens` has despite its comment); only consumption goes through this function, for atomicity, not grant necessity. |
