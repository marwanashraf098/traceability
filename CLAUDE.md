...
**Shopify Protected Customer Data — `shippingAddress` and `customer` fields require separate approval**: Adding `read_customers` scope alone is not enough. Shopify enforces a "Protected Customer Data" gate that blocks access to names, addresses, phone numbers, and emails even when the scope is present. To enable it: Shopify Admin → Settings → Apps and sales channels → [your custom app] → API credentials → regenerate token after scope change AND Partner Dashboard → Apps → [app] → App setup → Protected customer data → request access. Until approved, `ShopifyHttpGateway` omits `shippingAddress` from the orders GraphQL query and sets `customer_name`, `customer_phone`, `address` to null; the `raw` jsonb column preserves the full Shopify response for later backfill. **`financialStatus` was also removed in API 2024-04 — use `displayFinancialStatus` instead.**
