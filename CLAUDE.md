...
**MODE B AMENDMENT #2 (approved by Marawan 2026-09-23 — status: approved, NOT YET BUILT):** Traced may create Bosta deliveries of type 25 (CUSTOMER_RETURN_PICKUP) only, and only from an approved customer return request. Creation must be idempotent via a claim-before-call row (same pattern as `ShopifyInventoryService.claim`). `businessReference` must be the order's number so existing ingest links it. This does not license creating SEND, EXCHANGE (30), or any other delivery type, nor any other Bosta write.
