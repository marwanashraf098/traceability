...
**FR-17 INVENTORY INVARIANT (approved by Marawan — do NOT relax without explicit approval):** Traced only ever INCREMENTS Shopify inventory. Shopify owns the DECREMENT — it decrements on order creation. A Traced decrement would double-count and stop the merchant selling stock they actually have. There are exactly TWO increment triggers: (1) receiving session close (+N per variant at that session's location); (2) return inspection approved as AVAILABLE (+1 per piece at the inspection location). The 'available' edge of the `return_pending_inspection → available` ledger transition is the only return trigger — pieces going to 'damaged' MUST NOT increment. No other trigger may be added without explicit approval. Any future session that adds a third trigger or a decrement trigger — however "helpful" it seems — is introducing a double-count bug that directly costs the merchant money.
