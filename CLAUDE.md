...
**`DemoSeeder` (FR-DEMO, `com.traceability.demo`) is an approved, narrowly-scoped exception to "everything goes through `InventoryLedger`" — never extend this carve-out to any other caller.** `DemoSeeder.reseed()` writes `pieces`/`piece_events` directly via raw SQL on the BYPASSRLS owner connection, bypassing `InventoryLedger.transition()`/`batchReceive()` entirely. This is safe only because of what it is: a fixture loader for exactly one hardcoded tenant id (`DemoSeeder.DEMO_TENANT_ID`), not a business operation, gated by the same hard tenant-id assertion that gates every DELETE in the same transaction. It never runs against a real tenant, never runs under `app_user`/RLS, and every statement it issues carries the demo tenant id explicitly. `piece_events` is still INSERT-only at the DB-grant level for `app_user` (unchanged) — the owner/postgres connection `DemoSeeder` uses is not subject to that REVOKE, same as Flyway migrations aren't. Do not use this pattern to bypass `InventoryLedger` anywhere else, and do not widen `DemoSeeder`'s reach beyond the demo tenant.
...
**Fourteen built SECURITY DEFINER-class escape hatches (13 SQL functions + the `DemoSeeder.reseed()` Java hatch) — adding a fifteenth requires explicit approval**: `auth_lookup_user` (V1), `resolve_tenant_by_shop_domain` (V1), `lookup_refresh_token` (V3), `resolve_tenant_by_webhook_secret` (V5, approved 2026-06-14), `provision_tenant_from_shopify` (V14, approved 2026-06-19), `consume_magic_link` (V16), `verify_and_consume_reset_code` (V85, extended V86, approved 2026-08-31), `check_password_reset_throttle` (V86, approved 2026-08-31), `consume_shopify_oauth_state` (V87, approved 2026-08-31), `purge_expired_shopify_oauth_state` (V87, approved 2026-08-31), `DemoSeeder.reseed()` (V93, approved — FR-DEMO Day 1), `check_demo_rate_limit()` (V94, approved — FR-DEMO Day 2), `upgrade_custom_app_to_oauth` (V31, amended V32 — #13, omitted from this list until the 2026-09-23 inventory correction; SECURITY DEFINER since V31, no application caller today), and **#14 `resolve_tenant_by_portal_slug(slug)` (V100, approved 2026-09-23, BUILT Step 4a)**: returns `tenant_id` only (enabled slugs, case-insensitive); all subsequent returns-portal work (`PortalService`) runs via `TenantContext.runAs(tenantId, ...)` under RLS. Hatches #1–#7, #9, #13 and #14 are RLS bypasses; #8, #10, and #12 are `REVOKE`-restricted-table reads/sweeps on the caller's behalf (same review bar); #11 is a Java admin job on the existing BYPASSRLS `ownerDataSource` (same pool `ExceptionDigestJob`/`ExceptionImmediateAlertJob` already use), not a new SQL function — see blueprint.md §16.1 row 11 for why a SQL DEFINER function doesn't fit this one. The fifth hatch is justified because Path-2 Shopify-first install must atomically create tenant+owner+store — impossible under app_user+RLS because the new tenant's UUID is unknown before its INSERT (chicken-and-egg). Any future cross-tenant read must go through a named, code-reviewed `SECURITY DEFINER` function (or, for #11's pattern, an equally-scrutinized admin-connection job); no other bare `BYPASSRLS` connections in application code. Full enumeration in `docs/blueprint.md §16.1` — that table is the authoritative source; keep this count in sync with it.

**`bosta_districts` refresh — owner-connection writes, approved 2026-09-25 for THIS TABLE ONLY (Step 4c-2); not a tenant-data hatch and not counted above.** `bosta_districts` (V105) is global Bosta reference data (cities → zones → districts), shaped like `bosta_state_mappings`: no `tenant_id`, no RLS policy, no tenant rows. `app_user` has SELECT only (V105 revokes INSERT/UPDATE/DELETE/TRUNCATE). `BostaDistrictsRefreshService` writes it on the `@FlywayDataSource` owner pool (same pool as `ExceptionDigestJob`/JobRunr), fed by `GET {BOSTA_BASE_URL}/api/v2/cities/getAllDistricts` sent **without** an Authorization header; a 401/403 is logged as "needs auth" and nothing else happens — **never** use a tenant's API key for this endpoint. Districts Bosta stops returning get `pickup_available = false`, never deleted; an empty or failed fetch changes nothing. Scheduled by `BostaDistrictsRefreshJob` (daily 04:00 Africa/Cairo, plus an enqueued run at startup when the table is empty). Do not widen owner-connection writes to any other table on the strength of this approval.

**Bosta v2 READS (Step 4c-2) live in `BostaV2Client`, separate from `BostaHttpGateway`.** The global `bosta.api-version` stays `v0` and every existing v0 call is unchanged; v2 URLs are built as `{BOSTA_BASE_URL}/api/v2/...` in that class only. Two reads, no writes: `cities/getAllDistricts` (no auth, above) and `pickup-locations` (the tenant's decrypted key as the raw Authorization header, like v0; 401/403 → "Bosta didn't accept the connected API key for locations"). Explicit timeouts, no Resilience4j retry.

**Portal pickup booking is per tenant: `tenants.portal_pickup_booking` (V105, default false; Settings switch since Step 4c-3 — only switchable ON with an active Bosta account and a saved return location, stamps `portal_pickup_booking_since`) replaced the global `PortalService.PICKUP_BOOKING` constant.** The public config's `pickupBooking`, the merchant settings' `pickupBooking`/`portalPickupBooking`, and the portal lookup's `pickup` offer all read it. The pickup area offered to a customer is the delivery city (newest delivered forward leg's `raw.dropOffAddress.city._id`) and its pickup-available districts from `bosta_districts` (`PickupAreaService`, shared by lookup, submit and the merchant drawer) — city/zone/district names only, never the street address. Submission re-derives the offer from scratch and requires a district exactly when one was offered; the choice is stored as a text snapshot on `return_requests.pickup_*`.
...
**MODE B AMENDMENT #2 (approved by Marawan 2026-09-23 — BUILT 2026-09-25, Step 4c-3):** Traced may create Bosta deliveries of type 25 (CUSTOMER_RETURN_PICKUP) only, and only from an approved customer return request. Creation must be idempotent via a claim-before-call row (same pattern as `ShopifyInventoryService.claim`). `businessReference` must be the order's number so existing ingest links it. This does not license creating SEND, EXCHANGE (30), or any other delivery type, nor any other Bosta write — **no terminate, no edit** (a wrong booking is fixed by a person in Bosta; Traced flags it).

Implementation: `BostaV2Client.createReturnPickup()` (POST `/api/v2/deliveries?apiVersion=1`, tenant's raw key) is called ONLY from `ReturnPickupBookingService.book()` (the `ReturnPickupBookingJob`). **The create path has NO retry of any kind** — no Resilience4j decorator, no internal retry, `@Job(retries = 0)`, its own timeouts (connect 5 s, read 20 s); a duplicate POST books a second courier, and `ReturnPickupBookingTest` proves exactly one POST per attempt (revert-checked against the v0 retry decorator: 3 POSTs). The claim row is `return_requests.booking_status` (V106): a committed conditional UPDATE to `'pending'` only from NULL / `'failed'` before the call; 2xx+tracking → `'booked'` (request `'pickup_booked'`); 4xx/429/connection-never-made → `'failed'` (safe to retry); 5xx/timeout/reset/unreadable 2xx → `'failed_ambiguous'`, NEVER re-POSTed automatically — a person checks Bosta, then "It wasn't booked — retry" (→ `'failed'`) or "It was booked — enter tracking number" (checked via v0 `fetchDelivery`: type 25, this order's businessReference, created after the claim). Read-back after booking (`fetchDelivery`) → `booking_verified_at`, or `'needs_review'` naming the differing fields. The job is enqueued only after the approving transaction commits (`PickupBookingScheduler.enqueueAfterCommit`); the 10-minute sweeper books orphaned approvals decided after `tenants.portal_pickup_booking_since`, moves `'pending'` older than 15 min to `'failed_ambiguous'`, and retries unverified read-backs. The payload and the Bosta response are never logged (customer PII).

**Return legs (`shipment_leg='return'`): courier states update the shipment row only and never move pieces. Only an intake scan moves a delivered piece into `return_pending_inspection`.** Enforced inside `BostaWebhookJob.applyMappedState()` (shared by every caller) — the piece-transition step is skipped for a return leg while the shipment row, status history, PII and not-traced tagging still run. Root cause (2026-09-23): the piece query in that step is order-scoped (every `active`/`packed` allocation on the order), so a CRP reaching state 46 moved *every* delivered piece on the order to `return_pending_inspection` — including items the customer kept. Intake acceptance for a CRP'd order instead goes through `ShipmentLinkService.hasReturnLegAwaitingIntake()` (return leg non-terminal, or `returned` with `return_intake_completed_at IS NULL`); `return_intake_completed_at` is stamped when a return session is **closed** (not abandoned) on each leg that has scan evidence from that session (see below). A returned-but-unscanned leg raises the `return_leg_unscanned` exception after `tenants.return_unscanned_window_days`.

**An order can have MORE THAN ONE return leg (V104, Step 4c-1).** Only the forward leg is one-active-per-order (`ux_active_forward_shipment_per_order`); return legs are guarded only by the global `UNIQUE shipments.tracking_number`. A return piece carries no link to the leg it came back on (`piece_events.shipment_id` is the FORWARD shipment), so **which leg a return scan belongs to is decided in exactly one place: `ShipmentLinkService.returnLegScanEvidenceSql()` — canonical, never re-derive it.** Request rule (Step 4d-1, approved 2026-09-26), checked first: the leg has evidence when a `return_received` piece event carries the `request_id` of the return request whose `return_shipment_id` is this leg — exact per leg, no AWB or timing needed; it only ever ADDS evidence, legs without a request (or without an attributed scan) fall through to Rules 1–2 unchanged. Rule 1: the leg's own AWB was scanned in a non-abandoned return session that holds a `return_received` event for the order. Rule 2: the leg is the order's only unstamped return leg and a `return_received` event for the order exists at/after the leg's `created_at` — except that an event from a session in which another return leg's AWB was scanned belongs to that leg, and a leg still `created`/`with_courier`/`returning` gets no Rule 2 evidence while the order has another return leg (`terminated`/`cancelled` legs don't count as other legs). Otherwise: no evidence — never guess between legs. Used by `ReturnSessionService.close()` (stamping, and therefore the auto-close job), `resolveReturnLegIfComplete()` (only flips unstamped legs with evidence) and `RETURN_LEG_AWAITING_SCAN_SQL` (awaiting-scan callout + `return_leg_unscanned`). On a single-leg order Rule 2 is the pre-V104 order-wide rule. Displays (Step 4d-2): a leg linked to a return request counts / shows exactly its request's items (`listCrpReturns` pending count, session parcel cards); legs with no request keep the order-level grouping.

**Return requests after approval (Step 4d-1): `ReturnRequestLifecycle` (portal package) is the ONLY writer of `return_request_items.item_status`, post-approval request status and `return_request_events` (append-only; app_user INSERT/SELECT only).** It is not a bean — every caller builds it from its own JdbcTemplate so it joins the caller's transaction and works on an app_user connection. It never moves pieces (InventoryLedger stays the only piece-status writer); request ids go into `return_received` event METADATA, `piece_events.shipment_id` stays the forward shipment.
- **Reservation release (do not regress — the pre-4d-1 bug kept approved pieces reserved forever):** `return_request_items.active` is the one-live-item-per-piece flag and a V108 CHECK keeps it equal to `item_status IN ('awaiting','arrived')`. A final disposition (restocked / damaged — never `mismatch`) finishes its item immediately (`done`, active false) even while other items are awaited; rejecting, `rest-not-coming` and closing release every remaining item (awaiting → `not_coming`, arrived → `done` on close). Any new terminal path must release items the same way.
- **Linking a return leg:** tracking number first (`linkByTracking` in ingest, `saveBooked`, and the booking sweeper's race repair); only when no request holds the leg or its tracking number, hand-booked auto-match with `ReturnRequestLifecycle.linkCandidateSql` (exactly one candidate → linked; two or more → never guess, `return_link_ambiguous` exception; deferred while a Traced booking on the order is `pending`). One request per leg (partial UNIQUE on `return_shipment_id`). Merchant fix: `POST /return-requests/{id}/link-leg`.
- **Scan attribution (`ReturnSessionService.scanPiece`, DELIVERED only):** the piece's own awaiting item on an open request (approved / pickup_booked / received), else a same-variant substitute for the oldest such request when the piece is bound nowhere (the item's piece_id is swapped). Attributed scans are accepted regardless of the return window, `return_kind = request_return` (precedence exchange_match > request_return > crp_return > customer_after_delivery). Anything else is never attached to a request — at most an `unexpected_item_received` event.
- **Lifecycle:** received = no awaiting item and ≥ 1 arrived/done; refund_pending = additionally nothing still arrived; `closed` (V107) via `rest-not-coming` with nothing arrived (reason rest_not_coming) or `close` (no_refund / other) from approved / pickup_booked / received / refund_pending. `refunded` only via `mark-refunded` (from refund_pending, with ≥ 1 refund that isn't voided).
- **Refunds (Step 4d-2) — `return_refunds` is an append-only money ledger; never UPDATE or DELETE a row.** A mistake is corrected by a `kind='void'` row pointing at the refund (`voids_refund_id`, once per refund — partial UNIQUE); app_user has INSERT/SELECT only (V109 REVOKE) and `ReturnRefundsTest` scans src/main for any UPDATE/DELETE of the table. Refunds are recorded in received / refund_pending only, by owner / manager only (every refund endpoint answers 403 to a worker) and money never appears on worker screens (the session / parcel views carry no amounts). Traced records refunds made elsewhere — it never moves money and never writes a refund (or restock) to Shopify.
- **Suggested refund amount:** `RefundSuggestionService` — first source that prices every item wins: live Shopify `order(id:)` READ via `ShopifyOrderPriceGateway` (`discountedUnitPriceAfterAllDiscountsSet.shopMoney.amount`, pinned API version, read_orders) → stored REST `orders.raw.line_items` (price − Σ discount_allocations / quantity) → `variants.price` (approximate). Shipping excluded; any Shopify failure silently falls back; no source → empty amount; never log the order payload.
- **Alerts:** `refund_pending_overdue` / `return_items_overdue` predicates live in `ReturnRequestLifecycle` (`REFUND_OVERDUE_SQL`, `ITEMS_OVERDUE_SQL`) and are shared with the Requests list — never re-derive them.
...
**FR-21 §7 AMENDMENT TO THE FR-17 v2 INVARIANT ABOVE (approved by Marawan, Step 5 build gate; extended by Marawan 2026-08-23 for FR-13.x Void/Hold — see the paragraph below):** The automatic inventory sync (FR-17 v2) is increment-only and never decrements; `adjustInventoryQuantities` continues to reject non-positive deltas and is not modified. Decrements are sanctioned **only** through a **named, closed set of dedicated single-attempt gateway methods** — as of 2026-08-23: `pushStockTakeWriteOff` (stock-take write-off), `pushVoidCorrection` (FR-13.x void), `pushHoldEnter` (FR-13.x hold-enter). Adding a fourth requires the same explicit approval this set itself required — never `inventorySetOnHandQuantities`, never the increment-only sync path, never a shared general-purpose decrement helper (each method is separate code, no common "decrement" utility they funnel through). The original stock-take method is unchanged by this extension: `ShopifyGateway.pushStockTakeWriteOff()` / `ShopifyHttpGateway`, equal to the pieces transitioned to `lost` in a session, at the tenant's `is_fulfillment` location, called only from `StockTakeReconciliationService.finalizeSession()` (Phase A — claim, committed before any HTTP call) / `StockTakeShopifyPushJob` (Phase B — the push, no DB transaction around the call). Retry rule for stock-take: a definitive Shopify rejection (`ShopifyException`) is safe to auto-retry; an ambiguous/unconfirmed response (`ShopifyAmbiguousException` — timeout, no response) is NEVER auto-retried and is surfaced via `stock_take_shopify_syncs.status = 'failed_ambiguous'` for manual verification against Shopify (`referenceDocumentUri = traced://stock-take/{session_id}`). Every method in this named set makes exactly one HTTP attempt, no internal retry — the decrement is NOT idempotent at the business level (a duplicate call double-decrements) — and validates its negative delta before any network call.

**FR-13.x VOID / ON HOLD — Shopify decrement call sites (approved by Marawan 2026-08-23, extending FR-21 §7 above).** `PieceAdjustService.voidPiece()` and `PieceAdjustService.hold()` are the only two callers of the two new gateway methods below; nothing else may call them.
- `ShopifyGateway.pushVoidCorrection()` — void is a receiving-overcount/duplicate-entry correction, not a loss; it decrements only if the piece's originating receiving increment actually applied (checked via `shopify_inventory_adjustments` — `trigger_type='receiving_session'`, keyed by the piece's `receipt_id`, `status='applied'`). If that increment never fired, the on_hand count is already correct and the call is skipped — still recorded in `shopify_inventory_adjustments` (`trigger_type='void_correction'`, `status='skipped'`) for audit. Idempotency key = `piece_id` (void is terminal — a piece can only be voided once).
- `ShopifyGateway.pushHoldEnter()` — a currently-sellable piece leaving the sellable pool into `on_hold`. Idempotency key = `piece_id` + the hold event id (distinct from void — a piece can be held, released, and held again, so `piece_id` alone would collide across cycles).
- The reverse, `hold_exit` (`on_hold → available`), is **not** a decrement — it goes through the **existing** positive/increment path (`applyIncrementAdjustment`, same as receiving/return-inspection), trigger_type `hold_exit`. It is not part of this named decrement set and needs no approval extension.
- Escalating `on_hold → damaged|lost|destroyed` makes **no** Shopify call at all — the piece already left Shopify's sellable pool at hold-enter; a second decrement here would double-count. This is the bug this whole design is structured to prevent.
...
**Shipment `internal_state` is monotonic w.r.t. `'created'`: once a forward shipment has any with_courier-or-later history, no webhook may set `internal_state` back to `'created'` (resolves to `exception`). History records truth; the column is guarded.** Root cause of the 2026-08-23 incident: Bosta reuses provider codes 10/11/20 ("created") for both a genuine first dispatch AND a "back at hub / re-route" event on a shipment that has already moved — a static per-code `bosta_state_mappings` table can't disambiguate those two cases, so the fix lives at the write choke-point instead. `BostaWebhookJob.applyMappedState()`'s `UPDATE shipments` `internal_state` CASE has a second guard, independent of and evaluated after the existing FR-16 custody-lock branch (untouched): if the incoming mapped state is `'created'` AND `shipment_status_history` already has any row for this shipment with `internal_state <> 'created'` (with_courier, returning, exception, delivered, returned, lost, terminated, cancelled — any real progress, not just with_courier), the write resolves to `'exception'` instead. The guard reads `shipment_status_history` (append-only), never the live `internal_state` column — reading the column would be circular, since it's exactly the value this guard corrects. The `shipment_status_history` INSERT itself is never filtered — it keeps recording exactly what Bosta sent. Do not ship a `bosta_state_mappings` migration for codes 10/11/20 instead of/alongside this guard: with the guard in place there is no remaining case where those codes mapping to `'created'` misbehaves on a shipment that has never progressed (a genuinely pre-dispatch shipment repeating 10/11/20 should stay `'created'` — that's correct, not a bug).

**Pick `scan()` and `complete()` (`FulfillService`) must re-verify eligibility server-side via `shipment_status_history` (`ALREADY_SHIPPED`), never trusting the queue list filter alone.** `FulfillService.PICKABLE_ORDERS_FILTER` (the `GET /fulfill/queue` predicate) is a display-time filter, not a security boundary — nothing re-checked it at any write, which is how the 2026-08-23 incident order got scanned and packed after its shipment's `internal_state` was wrongly rewound to `'created'`. Both `scan()` and `complete()` now call a private `hasEverShippedPastCreated(orderId, tenantId)` guard as their first step (right after `TenantContext.require()`, before touching pieces/allocations): `SELECT EXISTS (... FROM shipment_status_history h JOIN shipments s ON s.id = h.shipment_id WHERE s.order_id = ? AND s.tenant_id = ? AND s.shipment_leg = 'forward' AND h.internal_state <> 'created')`. On a hit, `scan()` returns `ScanResult.rejected("ALREADY_SHIPPED", ...)` and `complete()` throws `ResponseStatusException(CONFLICT, ...)`. Deliberately reads history, not the live `shipments.internal_state` column, for the same circularity reason as the mapper guard above. Self-pickup orders never get a forward shipment, so the query naturally returns no rows for them — no carve-out needed. Deliberately does **not** gate on `not_traced_at` — that flag has its own known false-positive bug (a genuinely-traced order was seen carrying the chip) and is display-only until that's fixed separately; carrier-movement history is the trustworthy signal here, not the flag.
