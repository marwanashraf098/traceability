 ...
 *
 * An order is excluded from the queue when its latest shipment_leg='forward' shipment is
 * already sent to Bosta (internal_state <> 'created'). No shipment, or one still 'created',
 * leaves the order in the queue. This is the state-gate filter alone — deliberately NOT
 * coupled to not_traced_at (see FulfillService.getQueue() javadoc).
 *
 * Test inventory:
 ...
 *   e — no linked shipment at all → order stays in queue
 *   f — self_pickup_pending order (no shipment) stays in queue — Awaiting Collection regression
...
class QueueSendStateGateTest {
    ...
    @Test
    void e_noLinkedShipment_staysInQueue() {
