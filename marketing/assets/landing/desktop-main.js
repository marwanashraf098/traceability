...
(() => {
const PLANS=[
  {id:'free',name:'Free',limit:200,lim:'Up to 200 pieces a month · 1 user',feat:['A barcode for every piece, from the Shopify order','Scan with your phone: received, shipped, returned','The full story of every piece']},
  {id:'pro',name:'Pro',limit:Infinity,lim:'Unlimited pieces · up to 5 users',feat:['All of Free','Every courier in one timeline: Bosta, Mylerz, Aramex, your riders','Holds, cleaning states, alerts when a piece goes quiet','Shopify only sells what you can ship','Exports, and a person to call']}
];
...
const renderPlans=()=>document.querySelectorAll('[data-plans]').forEach(host=>{
  host.innerHTML=PLANS.map(p=>{ const pro=p.id==='pro'; const pr=pro?`<div class="price"><s>${fmt(PRICE[bill].v)}</s>EGP 0<small>${PRICE[bill].unit}</small>${PRICE[bill].was?'<span class="was">'+PRICE[bill].was+'</span>':''}</div><span class="now"><i></i>Free during launch</span>`:`<div class="price">Free<small></small></div><span class="now" style="visibility:hidden"><i></i>&nbsp;</span>`;
    return `<div class="plan ${pro?'pro':''}" data-plan="${p.id}"><span class="tag"><i class="dotslot"></i>For your numbers</span><h3>${p.name}</h3>${pr}<div class="lim">${p.lim}</div><ul>${p.feat.map(f=>'<li><i></i>'+f+'</li>').join('')}</ul><a class="pill ${pro?'':'q'}" href="https://app.tracedtech.com/signup">${pro?'Start Pro, free for now':'Start free'}</a></div>`; }).join('');
  document.querySelectorAll('.plan').forEach(p=>p.classList.toggle('rec',p.dataset.plan===(window.__rec||'pro')));
});
renderPlans();
...
document.querySelectorAll('[data-calc]').forEach(sec=>{
  ...
  const run=()=>{ const o=get('orders'), v=get('value'), r=get('ret'), l=get('lost'), rc=get('rec'), dg=get('dmg');
    ...
    const total=v1+v2; const plan=PLANS.find(p=>pieces<=p.limit); const price=plan.id==='pro'?PRICE[bill].v:0;
    const payDays=price?Math.max(1,Math.ceil(price/(total/30))):0; window.__rec=plan.id;
    ...
    count(total); set('mult',Math.max(1,Math.round(total/900))+'× ');
  ...
  (() => {
    ...
    const tick=()=>{
      ...
      if(t23>0){ const gy=Math.max(G.y,HOLD); x=lerp(A2.x,G.x,t23); y=lerp(A2.y,gy,t23)+Math.sin(t23*Math.PI)*-60; }
      ...
      const size=(t01>0||t12>0||t23>0||t34>0)?lerp(dsz,26,Math.max(t01,t12,t23,t34)):dsz;
      put(x,y,op,size);
      requestAnimationFrame(tick); };
    tick();
  })();
...
function go(i){ if(i===cur) return; cur=i; const b=BEATS[i]; const aur=document.getElementById('whyAur'); if(aur){ const P=[[0,0,1],[-3,-2,1.08],[3,2,1.04]][i]; aur.style.setProperty('--sx',P[0]); aur.style.setProperty('--sy',P[1]); aur.style.setProperty('--ss',P[2]); }
  ...
  b.cards.forEach((c,k)=>{ const el=document.createElement('div'); el.className='pc'; el.dataset.k=k; el.dataset.w=c.w; place(el,k,c.w); el.style.setProperty('--d',k*1.7); el.style.setProperty('--a',10+(k%3)*6); el.innerHTML=c.img?`<img src="${c.img}" alt="">`:c.html; field.appendChild(el); requestAnimationFrame(()=>setTimeout(()=>el.classList.add('on'),80+k*90)); }); }
